...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    selectInput("selectedstate","state",
                choices = statename,selected = "Missouri"
    ),
...
server <- function(input, output) {
  ...
  output$timeseries <- renderPlot({
    data=subset(crimerate,crimerate$STATE==input$selectedstate)
    plot(data[['YEAR']],data[[input$selectedoffense]],xlab="Year",ylab="Crime",type="b")
  })
  }
# Run the application 
